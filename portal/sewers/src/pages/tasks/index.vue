<template>
  <v-container class="w-75">
    <div class="d-flex align-center mb-6">
      ...
      >
      <h1 class="text-h4 font-weight-bold">Zarządzanie Zadaniami</h1>
      <v-spacer></v-spacer>
            ...
            <v-menu location="bottom end" transition="slide-y-transition">
              ...
              <v-list class="aero-glass-menu" elevation="10" density="compact">
                ...
                <v-list-item
                  @click="
                    () => {
                      config.editTask(task.id, {
                        name: task.name + ' (Edytowane)',
                      });
                      notificationStore.showSnackbar(
        ...
        <v-card-text class="pa-6 modal-body-bg">
          ...
          <div class="form-section mb-2">
            ...
            <div class="d-flex justify-space-between slider-labels mb-2">
              <span
                ><v-icon size="x-small" color="success">mdi-piggy-bank</v-icon>
                NISKI KOSZT</span
              >
              <span
                >MAX PRĘDKOŚĆ
  ...
  contextMenuStore.openMenu(event, [
    {
      ...
      action: () => {
        config.pauseTask(task.id);
        notificationStore.showSnackbar("Zadanie wstrzymane", "info");
      },
    },
    {
      label: "Zatrzymaj",
      action: () => {
        config.stopTask(task.id);
        notificationStore.showSnackbar("Zadanie zatrzymane", "warning");
      },
    },
    {
      label: "Edytuj nazwę",
      action: () => {
        config.editTask(task.id, { name: task.name + " (Edytowane)" });
        notificationStore.showSnackbar("Zadanie zaktualizowane", "success");
      },
    },
    {
      label: "Usuń",
      action: () => {
        config.removeTask(task.id);
        notificationStore.showSnackbar("Zadanie usunięte", "error");
      },
    },
  ]);
}
