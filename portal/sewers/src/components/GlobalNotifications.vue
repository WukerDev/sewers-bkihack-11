  ...
  <div>
    ...
    >
      ...
      <template v-slot:actions>
        <v-btn class="aero-text-main font-weight-bold" variant="text" @click="store.snackbar.show = false">
          Zamknij
        </v-btn>
      </template>
    </v-snackbar>
    ...
    <v-dialog v-model="store.dialog.show" max-width="500" persistent>
      <v-card class="aero-glass aero-window" elevation="10">
        ...
        <div class="aero-window-content">
          <v-card-text class="pt-4 aero-text-body">{{ store.dialog.text }}</v-card-text>
